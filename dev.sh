...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=twitter-djavue.example.com
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development twitter-djavue npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop twitter-djavue
    docker rm twitter-djavue
    docker run --name twitter-djavue -d --env-file /home/ubuntu/twitter-djavue.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/twitter-djavue:/dkdata \
        twitter-djavue start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./twitter-djavue/
...
echo_green "Welcome to twitter-djavue's dev env"
