...
```bash
dk bash  # starts bash inside "twitter-djavue" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and twitter-djavue.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `twitter-djavue` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The twitter-djavue container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv twitter-djavue -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database twitter-djavue owned by a user twitter-djavue.

On your remote machine, create a file ~/twitter-djavue.env:

```
DJANGO_DB_PASSWORD=<twitter-djavue's password>
DJANGO_DB_HOST=<database_ip>
DJANGO_DB_NAME=twitter-djavue
DJANGO_DB_USER=twitter-djavue
DJANGO_DEBUG=0
```
...
```
server {
    server_name  twitter-djavue.example.com;
    ...
    location /static {
        alias /home/ubuntu/dkdata/twitter-djavue/static;
...
(Replace "twitter-djavue.example.com" with your production domain)
